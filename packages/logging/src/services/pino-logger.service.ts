...
import { ILoggerService, LogContext, LogConfig, LogMetadata } from '../interfaces/logging.interface';
import { PinoLoggerConfigService, LogLevel } from './pino-logger-config.service';
...
export class PinoLoggerService implements ILoggerService, OnModuleDestroy {
	...
	getStats(): {
		...
		return {
			...
			averageLogSize: this.stats.totalLogs > 0 ? this.stats.totalLogSize / this.stats.totalLogs : 0,
	 ...
	 */
	private log(level: LogLevel, message: string, context?: LogContext, metadata?: LogMetadata, error?: Error): void {
		this.ensureLoggerInitialized();
	 ...
	 * @description 更新统计信息
	 */
	private updateStats(level: LogLevel, context: LogContext, logSize: number): void {
