...
export class PinoLoggerConfigService {
	...
	constructor(private customConfig?: Partial<LogConfig>) {
	...
	validateConfig(config: LogConfig): void {
		...
		if (config.rotation) {
			if (config.rotation.maxSize && !this.isValidFileSize(config.rotation.maxSize)) {
	...
	private initializeConfig(): void {
		...
		const defaultConfig: LogConfig = {
			...
			format: (process.env.LOG_FORMAT as LogFormat | undefined) ?? defaultFormat,
