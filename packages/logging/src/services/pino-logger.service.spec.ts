...
describe('PinoLoggerService', () => {
	...
	beforeEach(async () => {
			...
			providers: [
				{
					...
					useFactory: (
						...
						config: PinoLoggerConfigService,
						factory: PinoLoggerFactory,
						cls: ClsService
					) => {
						return new PinoLoggerService(emitter, config, factory, cls);
					},
		...
		configService = module.get<PinoLoggerConfigService>(PinoLoggerConfigService);
	...
	describe('basic logging', () => {
		it('should log debug messages', () => {
			...
			expect(spy).toHaveBeenCalledWith('debug', 'Debug message', LogContext.SYSTEM, { test: 'data' }, undefined);
		});

		it('should log info messages', () => {
			const spy = jest.spyOn(service as any, 'log');
			service.info('Info message', LogContext.BUSINESS, { operation: 'test' });
			expect(spy).toHaveBeenCalledWith('info', 'Info message', LogContext.BUSINESS, { operation: 'test' }, undefined);
		});

		it('should log warn messages', () => {
			const spy = jest.spyOn(service as any, 'log');
			service.warn('Warning message', LogContext.AUTH, { userId: '123' });
			expect(spy).toHaveBeenCalledWith('warn', 'Warning message', LogContext.AUTH, { userId: '123' }, undefined);
		});

		it('should log error messages', () => {
			const error = new Error('Test error');
			const spy = jest.spyOn(service as any, 'log');
			service.error('Error message', LogContext.DATABASE, { query: 'SELECT *' }, error);
			expect(spy).toHaveBeenCalledWith('error', 'Error message', LogContext.DATABASE, { query: 'SELECT *' }, error);
		});

		it('should log fatal messages', () => {
			const spy = jest.spyOn(service as any, 'log');
			service.fatal('Fatal message', LogContext.SYSTEM, { critical: true });
			expect(spy).toHaveBeenCalledWith('fatal', 'Fatal message', LogContext.SYSTEM, { critical: true }, undefined);
		});

		it('should log trace messages', () => {
			const spy = jest.spyOn(service as any, 'log');
			service.trace('Trace message', LogContext.PERFORMANCE, { duration: 100 });
			expect(spy).toHaveBeenCalledWith('trace', 'Trace message', LogContext.PERFORMANCE, { duration: 100 }, undefined);
		});
	});
	...
	describe('specialized logging', () => {
		it('should log performance messages', () => {
			...
			expect(spy).toHaveBeenCalledWith('info', 'Performance: database_query took 150ms', LogContext.DATABASE, {
				table: 'users',
		...
		it('should log business messages', () => {
			...
			expect(spy).toHaveBeenCalledWith('info', 'User registered', LogContext.BUSINESS, {
				userId: '123',
				email: 'test@example.com'
			});
		});
		...
		it('should log security messages', () => {
			...
			expect(spy).toHaveBeenCalledWith('warn', 'Failed login attempt', LogContext.AUTH, {
				ip: '192.168.1.1',
				username: 'test'
			});
		});
	...
	describe('configuration', () => {
		...
		it('should get and update config', () => {
			...
			jest.spyOn(configService, 'updateConfig').mockImplementation(() => undefined);
	...
	describe('child logger', () => {
		it('should create child logger', () => {
			...
			jest.spyOn(loggerFactory, 'createChildLogger').mockReturnValue(mockChildLogger as any);
			...
			expect(loggerFactory.createChildLogger).toHaveBeenCalledWith(expect.anything(), {
	...
	describe('statistics', () => {
		it('should track log statistics', () => {
			...
			expect(stats.logsByContext[LogContext.BUSINESS]).toBeGreaterThanOrEqual(1);
			expect(stats.logsByContext[LogContext.AUTH]).toBeGreaterThanOrEqual(1);
			expect(stats.logsByContext[LogContext.DATABASE]).toBeGreaterThanOrEqual(1);
