...
export class PinoLoggingMiddleware implements NestMiddleware {
	...
	private extractTenantId(req: FastifyRequest): string | undefined {
		...
		// 从查询参数中提取
		const queryTenantId =
			(req.query as Record<string, unknown>)?.tenantId || (req.query as Record<string, unknown>)?.tenant_id;
		if (queryTenantId) return String(queryTenantId);

		// 从请求体中提取
		const bodyTenantId =
			(req.body as Record<string, unknown>)?.tenantId || (req.body as Record<string, unknown>)?.tenant_id;
	...
	private extractUserId(req: FastifyRequest): string | undefined {
		...
		// 从查询参数中提取
		const queryUserId =
			(req.query as Record<string, unknown>)?.userId || (req.query as Record<string, unknown>)?.user_id;
		if (queryUserId) return String(queryUserId);

		// 从请求体中提取
		const bodyUserId = (req.body as Record<string, unknown>)?.userId || (req.body as Record<string, unknown>)?.user_id;
