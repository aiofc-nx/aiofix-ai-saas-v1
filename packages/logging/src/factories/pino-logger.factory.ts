...
export class PinoLoggerFactory {
	 ...
	 * @param {LogConfig} config 日志配置
	 */
	private configureTransport(pinoOptions: pino.LoggerOptions, config: LogConfig): void {
	 ...
	 * @param {LogConfig} config 日志配置
	 */
	private configureFileTransport(pinoOptions: pino.LoggerOptions, config: LogConfig): void {
	 ...
	 * @param {LogConfig} config 日志配置
	 */
	private configureRemoteTransport(pinoOptions: pino.LoggerOptions, config: LogConfig): void {
		...
		const remoteTransport = {
			...
			options: {
				...
				headers: {
					Authorization: config.remote.token ? `Bearer ${config.remote.token}` : undefined,
